...
vector<string> splitStrings(string str, char dl = ' ', int maxsplit = numeric_limits<int>::max()) {
    string word = "";
...
vector<string> resplit(const string & s, string rgx_str = "\\s+") {
    vector<string> result;
    regex rgx (rgx_str);
...
string run(const string &cmd, bool check=true, bool block=true) {
    // Run function and return standard output, Popen() handle, or None
    string result;
    try {
        if(block)
            result = exec(cmd.c_str(), check);
        else
...
vector<map<string, string>> get_devices_list(const string &cmd_list_devices, const string &device_list) {
    ...
    if(stdout != "") {
        ...
        while(getline(ss,tmp,'\n')){
            arr_stdout.push_back(tmp);
        }

        for(string line : arr_stdout) {
            trim(line);
            if(line != "" && line.find(":")) {
                vector<string> tmp = splitStrings(line, ':', 1);
                trim(tmp.at(1));
                dev.insert({tmp.at(0), tmp.at(1)});
            }
            else if(dev.size()) {
...
map<string, string> get_device_info(string name, string cmd_list_devices, string device_list) {
    ...
    for(map<string, string> d : devices) {
        if(d.find("Size") != d.end() && d["Capabilities"].find("pointer"))
            return d;
        for(regex txt : {regex("touch ?pad", regex_constants::icase), regex("track ?pad", regex_constants::icase)})
            for(map<string, string> d : devices) {
                smatch m;
                if(regex_search(d["Device"], m, txt))
                    return d;
            }
    }
...
map<string, string> get_device(string name, string cmd_list_devices, string device_list) {
    ...
    if(!dev.empty()) {
        ...
        dev["_path"] = devname;
        ostringstream stream;
        stream << devname << evname << ": " << dev["Device"];
        dev["_diag"] = stream.str();
    }
...
class COMMAND {
    ...
    public:
        ...
        COMMAND(const vector<string> arr_arg) {
            reprstr = join(arr_arg, ' ');
            // Expand '~' and env vars in executable command name
            ostringstream stream;
            stream << getenv("HOME") << "/" << arr_arg[0];
            replace(arr_arg.begin(), arr_arg.end(), arr_arg[0], expand_env(stream.str()));
            argslist = arr_arg;
        }
...
void add_internal_command(COMMAND cls) {
    ...
    regex e ("^COMMAND");
    string key = regex_replace(typeid(cls).name(), e, "");
...
class COMMAND_internal : public COMMAND {
    ...
    public:
        COMMAND_internal(const vector<string> arr_arg) :
        COMMAND(arr_arg),
        commands({
            {"ws_up", {0, 1}},
            {"ws_down", {0, -1}},
...
//            rows = 0;
//            cols = 0;
//            int cmdi = getIndex(commands_list, args.action);
...
//            }
        ...
        string Run(bool block = false) {
            ...
            int start, index = getIndex(lines, "*");
            int num = lines.size();
            int Cols = floor(((cols != 0) ? cols : num) / rows);
            ...
            count = ymove * Cols;
            if(count < 0) {
                if(index < Cols && nowrap) return "";
                index = (index + count) % numv;
                if(index >= num) index += count;
            }
            else if(count > 0) {
                index += count;
                if(index >= numv) {
    ...
    public:
        ...
        string add(string motion, string fingers, string command) {
            ...
            if(find(SUPPORTED_MOTIONS.begin(), SUPPORTED_MOTIONS.end(), motion) == SUPPORTED_MOTIONS.end()) {
                ostringstream stream;
                stream << "Gesture " << name << " does not support motion \"" << motion << "\"." << endl << "Must be \"" << join(SUPPORTED_MOTIONS, ',') << "\"";
                return stream.str();
            }
...
class SWIPE : public GESTURE {
    ...
    public:
        ...
        void end() override {
            ...
            if(pow(abx, 2) + pow(aby, 2) < abzsquare) return;
...
class PINCH : public GESTURE {
    ...
    public:
        PINCH() : GESTURE(),
            SUPPORTED_MOTIONS({
                "in", "out", "clockwise", "anticlockwise"
            }),
            extended_text("clock"){}
...
map<string, string(string)> conf_commands = {};
...
string conf_gesture(string lineargs) {
    ...
    if(handler) {
        vector<string> arr_handlers;
        for(auto const& [key, val] : handlers) {
            transform(key.begin(), key.end(), key.begin(), ::tolower);
            arr_handlers.push_back(key);
        }
        ostringstream stream;
        stream << "Gesture (" << gesture << ") is not supported.\nMust be (" << join(arr_handlers, ',') << ")";
        return stream.str();
    }
...
string read_conf(string conffile, string defname) {
    ...
    if(conffile != "") {
        ...
        if(!fs::exists(confpath)) {
            cerr << "Conf file (" << conffile << " does not exist.";
            exit(0);
        }
    }
    else {
...
int main(int argc, char *argv[])
{
    ...
    opt.addHelpOption();
    QCommandLineOption confOption(QStringList() << "c" << "conffile",
    ...
    QCommandLineOption verboseOption(QStringList() << "v" << "verbose", QCoreApplication::translate("main", "output diagnostic messages"));
    opt.addOption(verboseOption);
    QCommandLineOption debugOption(QStringList() << "d" << "debug", QCoreApplication::translate("main", "output diagnostic messages only, do not action gestures"));
    opt.addOption(debugOption);
    QCommandLineOption rawOption(QStringList() << "r" << "raw", QCoreApplication::translate("main", "output raw libinput debug-event messages only, do not action gestures"));
    opt.addOption(rawOption);
    QCommandLineOption listOption(QStringList() << "l" << "list", QCoreApplication::translate("main", "just list out environment and configuration"));
        ...
        if(!opt.isSet(rawOption)) {
            ...
            for(auto const& handler : handlers) {
                for(auto const& motion : handler.second->Motions()) {
                    ...
                    cout << handler.second->Name() << " " << _motion << " " << fingers << " " << motion.second;
                }
            }
...
//    cmd.stdout
    for(string line : splitStrings(command)) {
        ...
        if(line.find("GESTURE_") == string::npos) continue;
        ...
        if(event == "UPDATE") {
            if(handler) {
                ...
                if(!handler->update(resplit(params, "[^-.\d]+")))
                    cout << "Could not parse " << gesture << " " << event << ": " << params << endl;
            }
        }
        else if(event == "BEGIN") {
            ...
            if(handler)
                ...
                cout << "Unknown gesture received: " << gesture << "." << endl;
        }
        else if(event == "END") {
            ...
            cout << "Unknown gesture + event received: " << gesture << " + " << event << "." << endl;
    }
